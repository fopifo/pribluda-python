"""
Приблуда на python — StreamGrid v2.4: стриминг-детектор сеток роботов.
...
v2.4 (2026-09-09): ДОКРУТКА ФИЛЬТРА (вариант C+B):
  C — фильтр применяется только к сериям, где k>1 — БОЛЬШИНСТВО гэпов
    хвоста (фальшивые серии); серии с чистыми одиночными попаданиями
    не трогаются.
  B — для шумовых qty терпим ОДИН пропуск: требуем ≥80% гэпов хвоста
    с k=1 (вместо строгого запрета всех k>1).
  Это решает проблему POSI (настоящий робот qty=2-3 срезался фильтром):
  одиночный шумовой удар в середине чистой серии больше не убивает сигнал.
...
class StreamGrid:
    ...
    def _test(self, sym, side, c, ts):
        ...
        single = 0
        multi = 0
        # v2.4: считаем отдельно k=1 и k>1 для фильтра C+B
        for g in tail:
            k = round(g / p)
        ...
        # v2.4: ФИЛЬТР NOISY QTY (вариант C+B):
        # C — применяется только если multi > single (фальшивая серия,
        #     большинство пропусков); чистые серии не трогаем.
        # B — для шумовых qty терпим ≤20% пропусков (≥80% k=1);
        #     если пропусков больше — это не робот, а случайные сделки.
        if multi > single and self.noisy_qty:
            noisy_hit = any(q in self.noisy_qty.get(sym, set()) for q in c.qs)
            if noisy_hit:
                total = single + multi
                if single / total < 0.8:
                    return None
