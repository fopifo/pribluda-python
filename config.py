...
Два семейства пресетов:
  ...
  - "fast_strict" — "классический" алгоритм. Робота считаем роботом,
    ...
    НОВОЕ: добавлен допуск объёма qty_tolerance_lots = 2 лота и
    медианный анализ интервалов (time_window_sec=0.5 сек). Это
    позволяет ловить роботов с небольшим разбросом объёма и лучше
    переживать единичные выбросы интервала.
  ...
  - "twap_strict" — для паттерна "крупная заявка, нарезанная по
    ...
    interval_tolerance строже (0.03 вместо 0.1) и min_repeats выше (4
...
DETECTOR_PRESETS = {
    "fast_strict": {
        ...
        # НОВЫЕ ПАРАМЕТРЫ (переняты из старого скринера)
        "qty_tolerance_lots": 2,   # допуск объёма в лотах
        "time_window_sec": 0.5,    # окно для медианы / stability_ratio
    },
    "twap_strict": {
        "min_interval": 3.0,
        "max_interval": 60.0,
        ...
        # для TWAP допуск объёма не используется, но окно для медианы оставляем
        "time_window_sec": 0.5,
    },
}
